...
function CriarToDo(texto, prioridade, arrayToDo) {
  let task = new ToDo(texto, prioridade);
  let existe = arrayToDo.some(x => x == task);
  if(!existe){
    arrayToDo.push(task);
    return task;
  }
...
function AtualizarToDo(textoAntigo, textoNovo, arrayToDo) {
  let existe = false;
  arrayToDo.forEach(task => {
    if(task.Texto == textoAntigo){
      task.Texto = textoNovo;
      existe = true;
    }
  })
  return existe;
...
function PesquisarToDo(arrayToDo, texto) {
 let existe = arrayToDo.some(task => task.Texto == texto);
 return existe;
}
